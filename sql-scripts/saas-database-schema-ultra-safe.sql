-- SaaS Database Schema for VaryAI (Ultra Safe Version)
-- This script safely adds the necessary tables and features for a complete SaaS application
-- It handles all existing tables, columns, and indexes gracefully
...
-- Create index for better performance (only if table exists and column exists)
DO $$
BEGIN
...
-- Notifications Table - Handle existing table gracefully
DO $$
BEGIN
    -- Check if notifications table exists
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'notifications') THEN
        -- Create new table with is_read column
...
BEGIN
    ...
    -- Notifications indexes (only if is_read column exists)
...
-- Notifications RLS (only if table and is_read column exist)
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'notifications') 
       AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'notifications' AND column_name = 'is_read') THEN
...
-- Create trigger to update last activity on activity log insert (only if table exists)
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'activity_logs') THEN
