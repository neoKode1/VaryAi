-- Fix Authentication and Credit System Issues
-- This script addresses both authentication and credit system problems

-- =====================================================
-- 1. FIX USER_CREDITS TABLE SCHEMA
-- =====================================================

-- Drop the existing user_credits table if it exists (to avoid conflicts)
DROP TABLE IF EXISTS public.user_credits CASCADE;

-- Recreate user_credits table with correct schema
...
-- =====================================================
-- 3. ENSURE CREDIT_USAGE_LOG TABLE EXISTS
-- =====================================================
...
DROP POLICY IF EXISTS "Users can view own usage" ON public.credit_usage_log;
CREATE POLICY "Users can view own usage" ON public.credit_usage_log
    FOR SELECT USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Service role can manage all usage" ON public.credit_usage_log;
CREATE POLICY "Service role can manage all usage" ON public.credit_usage_log
    FOR ALL USING (auth.role() = 'service_role');

-- =====================================================
-- 8. CREATE CREDIT SYSTEM FUNCTIONS
-- =====================================================
...
-- =====================================================
-- 10. ENSURE ALL USERS HAVE CREDIT RECORDS
-- =====================================================
