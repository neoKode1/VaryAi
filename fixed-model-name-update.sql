-- Fixed model name updates to avoid duplicate key violations
...
-- Handle duplicates by removing the ones we don't want to keep
-- Keep the ones with better categories/costs and remove duplicates

-- Remove duplicate stable-video-diffusion entries (keep the one with better category)
DELETE FROM public.model_costs 
WHERE model_name = 'stable-video-diffusion-i2v' 
AND id NOT IN (
    SELECT id FROM public.model_costs 
    WHERE model_name = 'stable-video-diffusion-i2v'
    ORDER BY created_at ASC
    LIMIT 1
);

-- Remove duplicate modelscope entries
DELETE FROM public.model_costs 
WHERE model_name = 'modelscope-i2v' 
AND id NOT IN (
    SELECT id FROM public.model_costs 
    WHERE model_name = 'modelscope-i2v'
    ORDER BY created_at ASC
...
-- Remove duplicate text2video-zero entries
DELETE FROM public.model_costs 
WHERE model_name = 'text2video-zero-i2v' 
AND id NOT IN (
    SELECT id FROM public.model_costs 
    WHERE model_name = 'text2video-zero-i2v'
    ORDER BY created_at ASC
...
-- Remove duplicate cogvideo entries
DELETE FROM public.model_costs 
WHERE model_name = 'cogvideo-i2v' 
AND id NOT IN (
    SELECT id FROM public.model_costs 
    WHERE model_name = 'cogvideo-i2v'
    ORDER BY created_at ASC
...
-- Remove duplicate wan entries
DELETE FROM public.model_costs 
WHERE model_name = 'wan-v2-2-a14b-i2v-lora' 
AND id NOT IN (
    SELECT id FROM public.model_costs 
    WHERE model_name = 'wan-v2-2-a14b-i2v-lora'
    ORDER BY created_at ASC
...
-- Image-to-Video Models (avoiding duplicates)
UPDATE public.model_costs 
...
-- Handle zeroscope duplicates - remove one first
DELETE FROM public.model_costs 
